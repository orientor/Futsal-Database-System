...
def query_12(con, cur):
    ...
    total_no = input("Enter total number of phone numbers:")

    if(check_name_without_space(first_name) and check_name_without_space(middle_name) and check_name_without_space(last_name)
        and check_positive_int(total_no, "total_no")) == 0:
        return 0
...
def query_13(con, cur):
    ...
    query = f"INSERT INTO stadium(name, nomp, fpn) VALUES('{name}', {matches_played}, '{middle_name}', {fpn});"
    print(query)
    try:
        cur.execute(query)
    except Exception as e:
...
def query_8(con, cur):
    ...
    con.commit()
    print("Successfully Inserted into database")
>>>>>>> Added 4 insertion functions - need to debug tho
